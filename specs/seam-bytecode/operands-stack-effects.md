# SEAM bytecode operands and stack effects

Opcode id, immediate operands, table metadata, and stack-effect schemas define SEAM bytecode instruction behavior. Operand decoding belongs to validation, not runtime recovery.

Project evidence: `LOCKED_LANGUAGE_DESIGN.md` section 18, `seam_bytecode_opcodes.def`, `grammar/seam-bytecode-text.ebnf`.
...
Fixed-width integer and float operands use little-endian bytes in `.seam` image. SEAM bytecode text/disassembly uses source literals assembled to same values.
...
Stack-effect strings use `seam_bytecode_opcodes.def` notation.
