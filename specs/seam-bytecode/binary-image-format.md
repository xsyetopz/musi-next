...
`.seam` is the compiled SEAM bytecode image. It is the public build/cache/distribution artifact for compiled modules. SEAM bytecode text/disassembly is a separate readable tool format that may assemble into `.seam` or disassemble from `.seam`.

`.seam` image favors loader-friendly density over binary golf:
...
`.seam` image begins with exactly 40-byte header. Header = loader-probe data; semantic module data lives in sections.
...
No generic operand-count byte. Operands follow accepted opcode schema from `seam_bytecode_opcodes.def`.
