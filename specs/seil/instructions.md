...
Locked core SEIL instruction families have VM behavior. `specs/seil/opcodes.md` lists ids, mnemonics, operand schemas, stack-effect schemas from `seil_opcodes.def`.
...
SEIL is typed stack-effect instruction language. Instructions consume stack values, immediate operands, and module/body metadata. Semantics are VM-oriented, not Musi syntax, while preserving type/source relationship for analysis and tooling.
...
Callee origin lives in declaration metadata: SEIL body, intrin/runtime binding, extern/foreign binding, or core runtime target.
